...
//The 90 degree rotation of the Lee tile. Checks for conflict while repainting.

public class lLee extends Tetromino {

	int a;

	public lLee(int vx, int vy, Color c, int startX, int startY, int w, int h,
			int t, int or) {
		super(vx, vy, c, startX, startY, w, h, t, or);
		blocks.add(new point(startX, startY - 1, 0, 1, 1));
		blocks.add(new point(startX, startY, 0, 1, 1));
		blocks.add(new point(startX, startY + 1, 1, 0, 1));
		blocks.add(new point(startX - 1, startY + 1, 1, 1, 0));
	}
