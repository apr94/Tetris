...
public class Rectangle extends Tetromino {

	int a;

	public Rectangle(int vx, int vy, Color c, int startX, int startY, int w,
			int h, int t, int or) {
		super(vx, vy, c, startX, startY, w, h, t, or);
		blocks.add(new point(startX - 1, startY, 1, 0, 1));
		blocks.add(new point(startX, startY, 1, 0, 0));
		blocks.add(new point(startX + 1, startY, 1, 0, 0));
		blocks.add(new point(startX + 2, startY, 1, 1, 0));
	}
