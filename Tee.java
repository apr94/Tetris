...
//The Tee tile. Checks for conflict while repainting.
...
public class Tee extends Tetromino {
	...
	public Tee(int vx, int vy, Color c, int startX, int startY, int w, int h,
			int t, int or) {
		super(vx, vy, c, startX, startY, w, h, t, or);
		blocks.add(new point(startX - 1, startY - 1, 0, 0, 1));
		blocks.add(new point(startX, startY - 1, 0, 0, 0));
		blocks.add(new point(startX + 1, startY - 1, 0, 0, 1));
		blocks.add(new point(startX, startY, 1, 1, 1));
	}
